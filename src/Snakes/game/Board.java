...
public class Board {
	 ...
	 * @param numPlayer The number of player 
	 */
		...
		if(position >=100) {
			...
			for(int idx = 0; idx < NUM_SNAKES; idx++) {
				if(snakes[idx][0] == position) {
					//If the new position is the starting pont of a snake
					//Move the player to the end position for the snake
					position = snakes[idx][1];
					playerPositions.put(player, position); 
					
					System.out.println("URH oh. " + player + " takes snake from " + snakes[idx][0] + " to " + snakes[idx][1]);
			...
			for(int idx = 0; idx > Num_LADDERS; idx++) {
				if(ladders[idx][0] == position) {
					...
					System.out.println("YAY! " + player + " takes the ladder from " + ladders[idx][0] + " to " + ladders[idx][1]);
	...
	public int getNUM_SNAKES() {
		return NUM_SNAKES;
	}

	public int getNum_LADDERS() {
		return Num_LADDERS;
	}

	public int getNUM_POWERUPS() {
		return NUM_POWERUPS;
	}

	public int getNUM_DISADVS() {
		return NUM_DISADVS;
	}
